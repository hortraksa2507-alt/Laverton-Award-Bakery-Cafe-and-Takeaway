...
import { C, CATS, DEFAULT_MENU } from "./menuConstants.js";
import { FoodIcon, ItemPic, PHOTOS } from "./FoodIcon.jsx";
...
export default function PriceBoard() {
          ...
          if (r?.value) {
            ...
            if (Array.isArray(old) && old.length) {
              const merged = DEFAULT_MENU.map((d) => {
                ...
                return prev && typeof prev.price === "number" && prev.price > 0
                  ? { ...d, price: prev.price }
                  : d;
              });
              old.forEach((m) => {
                if (m.id && String(m.id).startsWith("x")) merged.push(m);
              });
              setMenu(merged);
              try {
                await store.set("laverton-menu-v4", JSON.stringify(merged));
              } catch (_) {}
              return;
            }
          }
        ...
        if (r?.keys?.length) {
          ...
          for (const k of r.keys) {
            try {
              ...
              if (g?.value) out[k.slice(9)] = g.value;
            } catch (_) { /* skip */ }
          }
          setUserPhotos(out);
        }
  ...
  const savePhoto = async (id, dataUrl) => {
    ...
    try {
      if (store) await store.set("labphoto-" + id, dataUrl);
    } catch (e) {
  ...
  const removePhoto = async (id) => {
    ...
    try {
      if (store) await store.delete("labphoto-" + id);
    } catch (_) {}
  };
  ...
  const shown = useMemo(() => {
    ...
    return menu.filter(
      (m) => (cat === "All" || m.cat === cat) && (!qq || m.name.toLowerCase().includes(qq))
    );
  }, [menu, cat, q]);
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ color: C.gold, fontSize: 10, letterSpacing: "0.22em", fontWeight: 700 }}>
            LAVERTON AWARD BAKERY
          </div>
          <div
            style={{
              fontFamily: DISPLAY,
      ...
      <div style={{ padding: "12px 16px 4px" }}>
        <input
          ...
          placeholder="Search… e.g. butter chicken"
          style={{
            width: "100%",
            background: C.panel,
        ...
        {["All", ...CATS].map((c) => (
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              cursor: "pointer",
              border: `1.5px solid ${cat === c ? C.gold : C.line}`,
              background: cat === c ? C.gold : "transparent",
              color: cat === c ? C.ink : C.muted,
            }}
          >
            {c}
          </button>
          ...
          return (
            ...
            >
              ...
              <div style={{ padding: "9px 11px 11px" }}>
                ...
                {unpriced ? (
                  <div style={{ color: C.red, fontSize: 12.5, fontWeight: 700, marginTop: 4 }}>
                    Tap to set price
                  </div>
                ) : (
                  <div
      ...
      <div style={{ textAlign: "center", paddingBottom: 24 }}>
        <button
          ...
          style={{
            background: "transparent",
            border: `1.5px solid ${resetArmed ? C.red : C.line}`,
            color: resetArmed ? C.red : C.muted,
            borderRadius: 10,
            padding: "9px 16px",
            fontSize: 12.5,
            cursor: "pointer",
          ...
          >
            {(() => {
              ...
              return src ? (
                ...
                >
                  {selItem ? (
                    <FoodIcon item={selItem} size={120} />
                  ) : (
                    <FoodIcon item={{ id: "?", cat: draft.cat }} size={120} />
                  )}
                </div>
              );
            })()}
              ...
              {!editing && selItem ? (
                <div>
                  ...
                  <div style={{ color: C.muted, fontFamily: MONO, fontSize: 14, marginTop: 6 }}>
                    ×2 {fmt(selItem.price * 2)} &nbsp;·&nbsp; ×3 {fmt(selItem.price * 3)} &nbsp;·&nbsp; ×4{" "}
                    {fmt(selItem.price * 4)}
                  </div>
                ...
                <div>
                  ...
                  {draft.photo && (
                    <div style={{ color: C.gold, fontSize: 12.5, marginTop: -6, marginBottom: 10 }}>
                      ✓ New photo ready — press Save to keep it
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
                    ...
                    <button
                      onClick={() =>
                        sel === "__new__"
                          ? close()
                          : (setEditing(false), setDraft((d) => ({ ...d, photo: null })))
                      }
                      style={{
                        background: "none",
                        border: `1.5px solid ${C.line}`,
